...
class UserService:
    ...
    async def get_by_email_or_phone(db: AsyncSession, username: str) -> User | None:
        """Lookup a user by email or mobile number (raw or normalized with +91)."""
        normalized_username = (
    ...
    @staticmethod
    async def create(db: AsyncSession, user_in: UserCreate, created_by_id: int | None = None) -> User:
        ...
        db_user = User(
            ...
            created_by=created_by_id,  # Audit: who triggered this creation
        )
        db.add(db_user)
        await db.flush()
        return db_user
    ...
    async def list_drivers(
        ...
        # Filter by vendor using the proper vendor_id column, not created_by
        if vendor_id:
